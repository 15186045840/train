...
<body>
				...
				<div class="row placeholders">
					<div>
						...
						<div class="modal fade " id="delete-confirm-dialog" tabindex="-1"
							...
							<div class="modal-dialog modal-sm" role="document">
								<div class="modal-content">
									...
									<div class="modal-body">确认删除所选广告吗</div>
						...
						<button type="button" class="btn btn-default show-add-form"
							data-toggle="modal" data-target="#advertisement-form-div">添加新广告</button>
						<!--添加新广告表单-->
						<div class="modal fade " id="advertisement-form-div" tabindex="-1"
							role="dialog" aria-labelledby="mySmallModalLabel">
								...
								<div class="modal-content">
										...
										<form class="advertisement-form">
											<div class="form-group">
												<label for="idInput">ID</label> <input type="text"
													name="aId" class="form-control" id="idInput"
													placeholder="广告ID">
											</div>
											<div class="form-group">
												<label for="typeInput">类型</label> <input type="text"
													name="aType" class="form-control" id="typeInput"
													placeholder="广告类型">
											</div>
											<div class="form-group">
												<label for="titleInput">标题</label> <input type="text"
													name="aTitle" class="form-control" id="titleInput"
													placeholder="广告标题">
											</div>
											<div class="form-group">
												<label for="pictureInput">图片</label> <input type="text"
													name="aPicture" class="form-control" id="pictureInput"
													placeholder="广告图片">
											</div>
											<div class="form-group">
												<label for="linkInput">链接</label> <input type="text"
													name="aLink" class="form-control" id="linkInput"
													placeholder="广告链接">
											</div>
										</form>
									</div>
									<div class="modal-footer">
										<button type="button" class="btn btn-default"
											data-dismiss="modal">取消</button>
										<button type="button"
											class="btn btn-primary advertisement-submit"></button>
									</div>
								</div>
							</div>
						</div>
					...
					<table class="table table-hover table-bordered advertisement-list">
						...
						<c:forEach items="${advertisements }" var="advertisement">
							<tr>
								<td><input type="checkbox" name="advertisementIds"
									value="${advertisement.aId }" /></td>
								<td class="advertisementid">${advertisement.aId }</td>
								<td>${advertisement.aType }</td>
								<td>${advertisement.aTitle }</td>
								<td>${advertisement.aPicture }</td>
								<td>${advertisement.aLink }</td>
								<td><a
									class="glyphicon glyphicon-pencil show-advertisementinfo-form"
									aria-hidden="true" title="修改广告信息" href="javascript:void(0);"
									data-toggle="modal" data-target="#advertisement-form-div"></a>
									<a class="glyphicon glyphicon-remove delete-this-advertisement"
									aria-hidden="true" title="删除广告" href="javascript:void(0);"></a></td>
							</tr>
						</c:forEach>
					</table>
				</div>
	...
	<script>
		...
		function resetAdvertisementForm(title, button) {
			...
			$(".advertisement-form input[type='checkbox']").prop("checked",
					false);
		...
		$(".select-all-btn").change(
				function() {
					if ($(this).is(":checked")) {//$(this).prop("checked")
						$(".advertisement-list input[type='checkbox']:gt(0)")
								.prop("checked", true);
					} else {
						$(".advertisement-list input[type='checkbox']:gt(0)")
								.prop("checked", false);
		...
		$(".advertisement-list").on(
				"click",
				".show-advertisementinfo-form",
				function() {
					resetAdvertisementForm("更新广告信息", "更新");

					var advertisementId = $(this).parents("tr").find(
							".advertisementid").html();
					$.ajax({
						...
						success : function(data) {
							$(".advertisement-form input[name='aId']").val(
									advertisementId);
							$(".advertisement-form input[name='aType']").val(
									data.aType);
							$(".advertisement-form input[name='aTitle']").val(
									data.aTitle);
							$(".advertisement-form input[name='aPicture']")
									.val(data.aPicture);
							$(".advertisement-form input[name='aLink']").val(
									data.aLink);
						}
					});
				});
							...
							if ($(this).html() === "添加") {
								...
								$
										.ajax({
											url : "add.html",
											type : "POST",
											data : $(".advertisement-form")
													.serialize(),
											dataType : "json",
											success : function(data) {
												$("#advertisement-form-div")
														.modal("hide");
												showTips("添加成功！");

												var newTr = '<tr>'
														+ '<td><input type="checkbox" name="advertisementIds" value="'+data.jId+'"/></td>'
														+ '<td class="advertisementid">'
														+ data.aId
														+ '</td>'
														+ '<td>'
														+ data.aType
		...
		$(".advertisement-list").on(
				...
				function() {
					...
					var advertisementId = advertisementTr.find(
							".advertisementid").html();
