  ...
  <body>
          ...
          <div class="row placeholders">
                    ...
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                      	<form class="role-form">
                            ...
                            <label for="roleIdInput">ID</label>
                            <input type="text" name="rleId" class="form-control" id="roleIdInput" placeholder="角色ID">
                          </div>
                          <div class="form-group">
                            <label for="roleNameInput">名称</label>
                            <input type="text" name="rleName" class="form-control" id="roleNameInput" placeholder="角色名">
                          </div>
                          <div class="form-group">
                            <label for="descInput">描述</label>
                            <input type="text" name="rleDescribe" class="form-control" id="descInput" placeholder="角色描述">
                          </div>
                          <div class="form-group">
                            <label for="codeInput">代码</label>
                            <input type="text" name="rleCode" class="form-control" id="codeInput" placeholder="角色代码">
                          </div>
                          <div class="form-group">
                            <label>设置权限</label>
                            <div class="jurisdiction-inputs"></div>
                          </div>
                        </form>
                      </div>
                      <div class="modal-footer">
            ...
            <table class="table table-hover table-bordered role-list">
                ...
                <c:forEach items="${roles }" var="role">
	                <tr>
	                	<td><input type="checkbox" name="roleIds" value="${role.rleId }"/></td>
	                    <td class="roleid">${role.rleId }</td>
	                    <td>${role.rleName }</td>
	                    <td>${role.rleDescribe }</td>
	                    <td>${role.rleCode }</td>
	                    <td><a href="javascript:void(0);" class="show-role-jurisdictions" >查看所有权限</a></td>
    ...
    <script>
    	function showTips(contents){
       	...
       	function getAllJurisdictions(obj){
        	obj.html("");
    		$.ajax({
				url:"listjurisdictions.html",
				type:"POST",
				dataType:"json",
		...
		$(".delete-selected-confirm").click(function(){
			...
			if(cbs.length===0){
				...
				$.each(cbs,function(i,jurisdiction){
					roleIds[i]=jurisdiction.value;
				});
				//请求删除所选角色
				$.ajax({
					url:"deletemore.html",
		...
		$(".show-add-form").click(function(){
			...
			getAllJurisdictions($(".jurisdiction-inputs"));
		});
		function getJurisdictionsByRoleId(roleId,doSuccess){
			$.ajax({
				url:"showrolejurisdictions.html",
				data:{roleId:roleId},
				type:"POST",
				dataType:"json",
				success:function(data){
		...
		$(".role-list").on("click",".show-roleinfo-form",function(){
			...
			$.ajax({
				...
				dataType:"json",
				success:function(data){
					$("input[name='rleId']").val(data.rleId);
					$("input[name='rleName']").val(data.rleName);
					$("input[name='rleDescribe']").val(data.rleDescribe);
					$("input[name='rleCode']").val(data.rleCode);
				}
			});
			getJurisdictionsByRoleId(roleId,function(data){
				for(var i in data){
					$('.role-form input[name="jurisdictionIds"][value="'+data[i].jId+'"]').prop("checked",true);
				}
			});
		});
			...
			if($(this).html()==="添加"){
				...
				$.ajax({
					...
					success:function(data){
						...
						var newTr=
							'<tr>'+
			                	'<td><input type="checkbox" name="roleIds" value="'+data.rleId+'"/></td>'+
			                    '<td class="roleid">'+data.rleId+'</td>'+
			                    '<td>'+data.rleName+'</td>'+
			                    '<td>'+data.rleDescribe+'</td>'+
			                    '<td>'+data.rleCode+'</td>'+
			                    '<td><a href="javascript:void(0);" class="show-role-jurisdictions" >查看所有权限</a></td>'+
			                    '<td><a class="glyphicon glyphicon-pencil show-roleinfo-form" aria-hidden="true" title="修改角色信息" href="javascript:void(0);" data-toggle="modal" data-target="#role-form-div"></a> '+
			                   		 '<a class="glyphicon glyphicon-remove delete-this-role" aria-hidden="true" title="删除角色" href="javascript:void(0);"></a></td>'+
